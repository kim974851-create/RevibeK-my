...
public class UserService {
    ...
    private UserMapper userMapper; // final 추가
    ...
    public UserResponseDto getUserById(String id) {
        ...
        UserResponseDto user = userMapper.selectUserById(id); // selectUserByEmail → selectUserById
        if (user == null) {
    ...
    public void updateUser(UserResponseDto dto) {
        ...
        UserResponseDto user = userMapper.selectUserById(dto.getId()); // selectUserByEmail → selectUserById
        if (user == null) {
            throw new RuntimeException("존재하지 않는 유저입니다");
        }
